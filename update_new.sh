...
docker stop xiaoya 2>/dev/null
docker rm xiaoya 2>/dev/null
docker rmi docker.teddy2022.tk/xiaoyaliu/alist:latest 
docker pull docker.teddy2022.tk/xiaoyaliu/alist:latest
if [[ -f /etc/xiaoya/proxy.txt ]] && [[ -s /etc/xiaoya/proxy.txt ]]; then
	proxy_url=$(head -n1 /etc/xiaoya/proxy.txt)
       	docker create  -p 5678:80 -p 2345:2345 -p 2346:2346 --env HTTP_PROXY="$proxy_url" --env HTTPS_PROXY="$proxy_url" --env no_proxy="*.aliyundrive.com" -v /etc/xiaoya:/data -v /etc/xiaoya/data:/www/data --restart=always --name=xiaoya docker.teddy2022.tk/xiaoyaliu/alist:latest
else
	docker create  -p 5678:80 -p 2345:2345 -p 2346:2346 -v /etc/xiaoya:/data -v /etc/xiaoya/data:/www/data --restart=always --name=xiaoya docker.teddy2022.tk/xiaoyaliu/alist:latest
fi	
